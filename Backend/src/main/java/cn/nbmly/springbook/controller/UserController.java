...
public class UserController {
    ...
    public Result<String> deleteUser(@PathVariable(required = false) String userId) {
        // 检查userId是否为undefined或无效值
        if (userId == null || "undefined".equals(userId)) {
            return Result.failed("无效的用户ID");
        }
        ...
        try {
            ...
            return Result.failed("用户ID必须是数字");
        }

        int res = userService.deleteUserByUserId(userIdInt);
     ...
     */
    @GetMapping("/{userId}")
    public Result<User> getUser(@PathVariable(required = false) String userId) {
        // 检查userId是否为undefined或无效值
        if (userId == null || "undefined".equals(userId)) {
            return Result.failed("无效的用户ID");
        }
    ...
    public Result<String> resetPassword(@PathVariable(required = false) String userId) {
        // 检查userId是否为undefined或无效值
        if (userId == null || "undefined".equals(userId)) {
            return Result.failed("无效的用户ID");
        }
        ...
        int res = userService.resetPassword(userIdInt);
