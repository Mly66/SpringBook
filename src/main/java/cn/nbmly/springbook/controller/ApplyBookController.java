...
public class ApplyBookController {
     ...
     */
    @GetMapping("/{bookId}")
    public Result<ApplyBook> getApplyBook(@PathVariable int bookId, HttpSession session) {
        ...
        ApplyBook applyBook = applyBookMapper.findApplyBookByBookIdAndUserId(bookId, userId);
    ...
    public Result<String> editApplyCount(@PathVariable int bookId, @RequestBody ApplyBook applyBook,
            HttpSession session) {
        String loginName = (String) session.getAttribute("loginName");
        if (loginName == null) {
            return Result.unauthorized();
        ...
        applyBook.getBook().setBookId(bookId);
    ...
    public Result<String> approveApply(@PathVariable int bookId, @RequestBody ApplyBook applyBook,
            HttpSession session) {
        String loginName = (String) session.getAttribute("loginName");
        if (loginName == null) {
            return Result.unauthorized();
        ...
        if (applyBook.getUser() != null && applyBook.getUser().getUserId() > 0) {
            existingApply = applyBookMapper.findApplyBookByBookIdAndUserId(bookId, applyBook.getUser().getUserId());
        } else {
            // 如果没有指定用户，尝试获取该书的第一条未审批记录
            List<ApplyBook> pendingApplies = applyBookMapper.findNoApprovedApplyBooksByBookId(bookId);
    ...
    public Result<String> inStorageApply(@PathVariable int bookId, HttpSession session) {
        String loginName = (String) session.getAttribute("loginName");
        if (loginName == null) {
            return Result.unauthorized();
        }
